...
export class dbApiService {
    ...
    getAll() {
        ...
        let response = fetch(`${this.apiUrl}/${this.props.url}`, requestOptions)
                .then(handleResponse, handleFetchError);
        return response;
    }

    getOne(id) {
        console_debug_log('dbServices::getOne() - id: '+id);
        const requestOptions = { method: 'GET', headers: this.props.authHeader };
        let response = fetch(`${this.apiUrl}/${this.props.url}?id=${id}`, requestOptions)
                .then(handleResponse, handleFetchError);
        return response;
    }
